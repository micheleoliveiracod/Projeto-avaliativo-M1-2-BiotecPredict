...
const Dashboard: React.FC<DashboardProps> = ({ refreshInterval = 30000 }) => {
  ...
  const [riskPrediction, setRiskPrediction] = useState<'LOW RISK' | 'MEDIUM RISK' | 'HIGH RISK'>('LOW RISK')
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch batches list
      const batchesResponse = await fetch('http://localhost:8000/api/v1/batches')
      ...
      if (batchesArray.length > 0) {
        ...
        const complianceResponse = await fetch(
          `http://localhost:8000/api/v1/compliance/${latestBatch.id}`
        )
        if (complianceResponse.ok) {
          const complianceData = await complianceResponse.json()
          const score = complianceData.score || complianceData.compliance_score || 0
        ...
        const predictionResponse = await fetch(
          `http://localhost:8000/api/v1/prediction/${latestBatch.id}`
        )
        if (predictionResponse.ok) {
          const predictionData = await predictionResponse.json()
          setRiskPrediction(predictionData.prediction || 'LOW RISK')
          setConfidenceScore(predictionData.confidence_score || 0)
        }

        // Set sensor data from latest batch
        if (latestBatch.sensor_data) {
          setSensorData(latestBatch.sensor_data)
        }
      }
    } catch (err) {
  ...
  const applyFilters = () => {
    ...
    if (filters.scoreRange !== 'all') {
      filtered = filtered.filter(batch => {
        const score = batch.compliance_score
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-header">
        <h1 className="dashboard-title">Dashboard Unificado</h1>
        ...
        <div className="kpi-card risk-card">
          ...
          <div className="card-content">
            ...
            <div className="risk-description">
              <p className="description-text">
                {riskPrediction === 'LOW RISK' && 'Processo dentro dos parâmetros esperados'}
                {riskPrediction === 'MEDIUM RISK' && 'Desvios moderados detectados'}
                {riskPrediction === 'HIGH RISK' && 'Risco significativo de falha de processo'}
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {paginatedBatches.map((batch) => (
                  <tr key={batch.id} className="table-row">
                    ...
                    <td className="table-cell">{(batch.confidence_score * 100).toFixed(1)}%</td>
                    <td className="table-cell">
