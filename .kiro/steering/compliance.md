...
| `compliance_score` | `batch` | Cálculo determinístico (backend) |
| `risk_prediction` | `prediction` | RandomForestClassifier (ML) |
| `batch_id`, `upload_date` | `batch` | Sistema (timestamp) |

**Metadados obrigatórios em `prediction`:**
...
|---|---|---|
| `batch` | 2 anos | Histórico de manufatura |
| `sensor_reading` | 2 anos | Análise de tendências |
| `prediction` | 2 anos | Validação de modelo |
...
|---|---|---|
| `batch_validation_errors.csv` | Após cada processamento | Batches rejeitados com motivo |
| `prediction_accuracy.csv` | Sob demanda | Acurácia do modelo por período |
| `data_quality_report.md` | Sob demanda | Visão completa: qualidade de dados, anomalias |

> Os CSVs não são versionados no Git (`.gitignore`).
...
- Banco PostgreSQL local não versionado
