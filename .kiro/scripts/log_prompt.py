...
# Cache global para evitar múltiplas chamadas a Git
_git_cache = {}


def _run_git_command(cmd, timeout=2):
    """
    Executa comando Git com timeout reduzido para performance.
    
    Args:
        cmd (list): Comando Git a executar
        timeout (int): Timeout em segundos (padrão: 2s, reduzido de 5s)
    
    Returns:
        str: Output do comando ou None em caso de erro
    """
    try:
...
def get_git_branch():
    """
    Retorna a branch Git atual (com cache para performance).
    ...
    Otimizações:
        ...
        - Fallback gracioso para 'unknown-branch'
    """
    if 'branch' in _git_cache:
        return _git_cache['branch']
    
    branch = _run_git_command(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], timeout=1)
...
def get_git_user():
    """
    Retorna o nome do usuário Git configurado (com cache para performance).
    ...
    Otimizações:
        - Cache em memória para evitar múltiplas chamadas
        - Timeout reduzido para 1 segundo
        - Fallback gracioso para 'Unknown User'
    """
    if 'user' in _git_cache:
        return _git_cache['user']
    
    user = _run_git_command(['git', 'config', 'user.name'], timeout=1)
    user = user if user else 'Unknown User'
    _git_cache['user'] = user
    return user
...
def sanitize_branch_name(branch):
    ...
    Também converte para minúsculas conforme convenção do projeto.
    ...
    Transformações aplicadas:
        - Converte para minúsculas (conforme convenção)
        - '/' → '-' (separadores de diretório em branches como feature/auth)
        - '\\' → '-' (separadores de diretório no Windows)
        - Espaços → '-'
        - Caracteres especiais → '-'
        - Hífens duplicados → hífen único
        - Hífens no início/fim → removidos
    ...
    # Converte para minúsculas (conforme convenção do projeto)
...
def log_prompt():
    """
    Função principal de logging de prompts (otimizada para performance).
    ...
    Otimizações para performance:
        - Cache de metadados Git (evita múltiplas chamadas)
        - Timeout reduzido em subprocess (2s em vez de 5s)
        - Operações de I/O otimizadas (batch write)
        - Fallback gracioso para não bloquear Kiro
    ...
    """
    try:
        # 1. Coleta metadados (com cache para performance)
        ...
        # 4. Cria diretório se não existir (com tratamento de erro rápido)
        try:
        ...
        # 5. Verifica se arquivo existe (mais rápido que tentar abrir)
        file_is_new = not os.path.exists(log_file)
        
        # 6. Formata entrada de log (antes de abrir arquivo)
        entry = format_log_entry(branch, user, timestamp, prompt_content)
        
        # 7. Escreve no arquivo (operação única de I/O)
        try:
            if file_is_new:
                # Escreve cabeçalho + primeira entrada em uma operação
                header = f'# Prompt Logs: {branch}\n\nHistórico de prompts executados no Kiro nesta branch.\n\n---\n\n'
            ...
            # Falha ao escrever - registra erro mas não bloqueia
            return
        ...
        # Não registra nem levanta exceção - falha graciosamente
        pass
